...
export default {
  data() {
    return {
      ...
      usaging: {title: "使用状況"},
      endpoint: "http://100.119.204.18:5000/3go/usages",
      lineColors: {
  ...
  methods: {
    async fetchLogs() {
      try {
        const response = await axios.get("http://100.119.204.18:5000/3go/logs");
    ...
    async fetchStatus(){
      try {
        const infos = await axios.get("http://100.119.204.18:5000/3go/info");
